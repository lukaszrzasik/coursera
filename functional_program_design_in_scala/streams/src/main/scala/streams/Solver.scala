...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.foldLeft(LazyList[(Block, List[Move])]())((lazyList: LazyList[(Block, List[Move])], blockAndMove: (Block, Move)) => (blockAndMove._1, blockAndMove._2 :: history) +: lazyList
    )
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList()
      ...
      neighbors ++ from(initial.tail ++ neighbors, explored ++ (neighbors map (_._1)) + initial.head._1)
    }
  }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart filter (_._1 == Block(goal, goal))
  ...
  lazy val solution: List[Move] = {
    ...
    else (pathsToGoal apply 0)._2.reverse
  }
}
